...
// now loads `../src/index.ts` directly (no build), the same specifier the
// product resolves at runtime — and that raw-TS worker load only resolves
// under a real Bun worker thread, confirmed empirically; vitest's own
// worker pool does not give the spawned `new Worker()` Bun's module loader,
// even when vitest itself runs under `bun run`.
export default [
