...
- [x] Playwright canvas e2e: rigid-body, zoom semantics, mid-drag refusal, arrangement durability — _landed against a real spawned server and real Chromium (`apps/web/e2e/canvas-rigid-body.spec.ts`, `canvas-zoom-containers.spec.ts`, `canvas-mid-drag-refusal.spec.ts`, `canvas-arrangement-durability.spec.ts`): rigid-body push chains through a drag and the settled arrangement measurably does not drift afterward; zoom switches the workstream-card/inner/detail renderers with real wheel-zoom, checked at every level; mid-drag refusal asserts the target handle's own `valid` class absent *while the mouse is still down* over an illegal content→content or content→ended-session target (plus a positive control proving the same detection fires legal both ways), and that the drop creates no edge client- or server-side; arrangement durability is proven at the layer that actually implements it — `PATCH /api/arrangement` surviving a real kill-and-respawn of the server on the same state dir — plus "reset arrangement" through the real UI. **Two honest findings surfaced while writing these, both left unfixed by design (outside this batch's file ownership) and reported to the routing agent:** (1) `ContainerNodeView` renders no `Handle` elements, so an edge remapped to a collapsed container's frame (Epic 3.2's own "edges draw to collapsed frames" claim) never actually renders — confirmed directly, fix routed separately to the active `packages/ui` writer along with its own render-level proof; the zoom e2e above deliberately omits an edge-to-frame assertion rather than assert the broken behavior. (2) The renderer (`apps/web/src/App.tsx`) still persists positions only to browser `localStorage`, exactly as Epic 3.1's own landed note already recorded ("the renderer still writes to localStorage until it adopts those endpoints") — the arrangement-durability spec proves the real server-side guarantee directly rather than a browser-only illusion of one, and is structured for a follow-up to add the browser-level leg once the renderer adopts the endpoints — that follow-up is now real: the arrangement-server-adoption branch landed `arrangement-durability.spec.ts` (a drag survives a real server restart into a brand-new browser context, a rigid-body push persists through the batch endpoint, and reset arrangement is durable server-side) and `arrangement-container-collapse.spec.ts` (the render-level edge-to-frame regression leg, fixed alongside)._
- [x] Multi-session steering e2e: inject, answer question, stop scopes, queue triage — _the W14 gate (`apps/web/e2e/steering.spec.ts`) already covers inject + question-from-bubble + stop-at-three-scopes in one run; hardened here with two new, additive spec files (`steering-hardening.spec.ts`, `steering-triage-restart.spec.ts`) that extend coverage rather than repeat it: a replayed injection id never doubles the ledger and a refused injection leaves no phantom row; a question answered from the QUEUE (never the bubble) settles the same blocked call as the bubble path and the same session's bubble reflects the identical answered state afterward (one vocabulary), with §6.4's unpicked option asserted `toBeVisible()` directly; a stopped session's own rendered text is asserted to never say "failed" and its card offers an enabled resume (§6.7, §3.6); a session broadcast reaches a second session sharing its repository (both workstreams' workspaces resolve to the harness's one configured repo) with the §6.5 rate window proven as an honest 409 refusal on the fourth send, never a silent drop, and the operator's own broadcast proven unconstrained by that same bound; and mute/snooze/acknowledge are proven distinct (mute stays hidden, snooze hides only until its own return time, acknowledge hides once immediately) plus durable across a real kill-and-restart of the server on the same state directory. Two gaps found and reported rather than forced: (1) acknowledging a drift row does not currently resurface it after a further change — `AttentionService#driftSources` reads `deriveBoardDrift(...).attention`, which `deriveDrift` (`packages/core/src/sessions/drift.ts`) filters by the acknowledge verb alone, so the baseline-aware re-drift check one level up never reaches the queue; confirmed by direct inspection, not asserted as either passing or failing behavior, and flagged in `steering-triage-restart.spec.ts`'s own doc comment for a decision outside this batch's file ownership. (2) the scripted runtime's blocked-request ids (`ask-`/`req-`/`call-${pendingSize}-${actIndex}`, `apps/server/src/runtime/scripted.ts`) are unique per session instance but not across sessions, so two independently-started sessions each raising their first blocking request in their first act collide at `SessionQuestion#forRequest`'s cross-session lookup (`packages/db/src/question-store.ts`) — worked around test-side (`askOnInjectionScript`), documented in `steering-hardening.spec.ts` rather than patched. Both gaps are outside this batch's ownership (`apps/server/src/attention/`, `packages/core/src/sessions/`, `apps/server/src/runtime/`, `packages/db/`). Verified: `pnpm verify` green, and the full Playwright e2e suite (17 tests across every spec, existing gates included) green across 5 consecutive runs._
